        ...
        <div className={styles.container1}>
          <Skeleton isLoading={!product}>
            <button
              ...
              onClick={() => {
                deleteCartProduct(String(prepareProduct.id));
              }}
            >
          ...
          <Skeleton isLoading={!product}>
            <Link to={`/${prepareProduct.category}/${prepareProduct.id}`}>
              <img className={styles.image} src={apiFromServer.getImageUrl(prepareProduct.generalImage)} alt="Image product" />
            </Link>
          </Skeleton>
          <div className={styles.nameContainer}>
            <Link
              className={styles.nameLink}
              to={`/${prepareProduct.category}/${prepareProduct.id}`}
            >
              <Skeleton isLoading={!product}>
          ...
          <Skeleton isLoading={!product}>
            <div className={styles.countContainer}>
              <ButtonSecond
                ...
                onClick={() => {
                  setCountCartProduct(String(prepareProduct.id), (prev) => prev - 1);
                }}
              >
